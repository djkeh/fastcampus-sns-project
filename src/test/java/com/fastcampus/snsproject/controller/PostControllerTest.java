...
public class PostControllerTest {
    ...
    void 포스트수정() throws Exception {
        ...
        when(postService.modify(eq(title),eq(body),any(),any()))
                .thenReturn(Post.fromEntity(PostEntityFixture.get("userName",35,24)));
//                .thenReturn(Optional.of(PostResponse.class));
...
//                .andExpect(status().isUnauthorized())
    ...
    void 포스트수정시_본인이_작성한_글이_아니라면_에러발생() throws Exception {
        ...
        // mocking(String title, String body, String userName, Integer postId)
        // TODO : 구현
    ...
    void 피드목록_요청시_로그인하지_않은경우() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/posts/1/my")
                        .contentType(MediaType.APPLICATION_JSON)
                ).andDo(print())
                .andExpect(status().isUnauthorized());
    }
    ...
    @Test
    @WithAnonymousUser
    void 종아요버튼_클릭시_로그인하지_않은경우() throws Exception {
    ...
    @Test
    @WithMockUser
    void 종아요버튼_클릭시_게시물이_없는경우() throws Exception {
        doThrow(new SnsApplicationException(ErrorCode.POST_NOT_FOUND))
    ...
    void 댓글기능() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/posts/49/comments")
                        .contentType(MediaType.APPLICATION_JSON)
