...
import com.fastcampus.snsproject.exception.SnsprojectApplicationException;
...
public class PostControllerTest {
    ...
    void 포스트수정시_본인이_작성한_글이_아니라면_에러발생() throws Exception {
        ...
        doThrow(new SnsprojectApplicationException(ErrorCode.INVALID_PERMISSION)).when(postService).modify(eq(title), eq(body), any(), eq(1));
    ...
    void 포스트수정시_수정하려는_글이_없는경우_에러발생() throws Exception {
        ...
        doThrow(new SnsprojectApplicationException(ErrorCode.POST_NOT_FOUND)).when(postService).modify(eq(title), eq(body), any(), eq(1));
    ...
    void 포스트삭제시_작성자와_삭제요청자가_다를경우() throws Exception {
        doThrow(new SnsprojectApplicationException(ErrorCode.INVALID_PERMISSION)).when(postService).delete(any(), any());
    ...
    void 포스트삭제시_삭제하려는_포스트가_존재하지_않을_경우() throws Exception {
        doThrow(new SnsprojectApplicationException(ErrorCode.POST_NOT_FOUND)).when(postService).delete(any(), any());
    ...
    void 좋아요버튼클릭시_게시물이_없는경우() throws Exception {
        doThrow(new SnsprojectApplicationException(ErrorCode.POST_NOT_FOUND)).when(postService).like(any(), any());
    ...
    void 댓글작성시_게시물이_없는경우() throws Exception {
        doThrow(new SnsprojectApplicationException(ErrorCode.POST_NOT_FOUND)).when(postService).comment(any(), any(), any());
