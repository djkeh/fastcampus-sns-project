...
public class PostControllerTest {
    ...
    @Test
    @WithAnonymousUser
    void 포스트작성_실패_로그인안함에러발생() throws Exception {
        mockMvc.perform(post("/api/v1/posts")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(new PostCreateRequest("title", "body"))))
                .andDo(print())
    ...
    @Test
    @WithAnonymousUser
    void 포스트수정_실패_로그인한상태아님() throws Exception {
    ...
    @Test
    @WithMockUser
    void 포스트수정_실패_본인이_작성한_글이_아니라면_에러발생() throws Exception {
        doThrow(new SnsApplicationException(ErrorCode.INVALID_PERMISSION)).when(postService).modify(any(), any(), eq("title"), eq("body"));
        mockMvc.perform(put("/api/v1/posts/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(new PostModifyRequest("title", "body"))))
                .andDo(print())
